...
/**
 * Class LoadPreviousChatItemsForm.
 */
class LoadPreviousChatItemsForm extends FormBase {
   ...
   */
  public function __construct(OlGroups $groups, OlChat $chat) {
    $this->groups = $groups;
    $this->chat = $chat;
  }
  ...
  public function submitChatAjax(array &$form, FormStateInterface $form_state) {
    ...
    $new_items = $this->chat->getChatItemsQueryLength();
    $offset_new = (int)$offset + $new_items;
    // Get html via chat service.
    $chat_data = $this->chat->getChatList($group_uuid, $offset_new);
    ...
    $response->addCommand(new AfterCommand('#append_here', $chat_html));
